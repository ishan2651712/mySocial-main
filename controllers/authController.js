...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.blacklisted) {
    return next(
      new AppError(
        "Your account is blacklisted, you cannot access this feature",
        401
      )
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetURL = `${req.protocol}://${req.get(
    "host"
  )}/social/users/resetPassword/${resetToken}`;
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!user || !(await user.correctPassword(password, user.password))) {
    ...
    return next(
      new AppError("Your New Password cannot be the same as the older one", 401)
    );
  }
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save({ validateBeforeSave: true, new: true, context: "query" });
